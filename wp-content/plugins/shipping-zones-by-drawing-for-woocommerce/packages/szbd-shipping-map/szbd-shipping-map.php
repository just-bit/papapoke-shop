...
// We have now started using dynamic load of google     
if(false){
    wp_register_script('szbd-google-autocomplete-2', 'https://maps.googleapis.com/maps/api/js?&callback=Function.prototype&libraries=geometry,places&types=address' . '' . '&key=' . $google_api_key,array(
        'jquery'
    ), SZBD_SHIPPING_MAP_VERSION, array('strategy' => 'defer'));
...
add_action('woocommerce_blocks_loaded', function () {
    ...
    add_action(
        ...
        function ($order, $request) {
            ...
            $lat = $request['extensions']['szbd']['point']['lat'];
            $lng = $request['extensions']['szbd']['point']['lng'];
            $pluscode = $request['extensions']['szbd']['pluscode'];
            $doSave = false;

            if(!is_null($lat) && !is_null($lng)){
                $latlng= json_encode(array('lat' => $lat, 'lng'=> $lng), JSON_FORCE_OBJECT);
    
                 $order->update_meta_data('szbd_picked_delivery_location', $latlng );
                 $doSave = true;
            }
    ...
    add_action(
        ...
        function ($customer, $request) {
            ...
            if(!is_null($lat) && !is_null($lng)){
                $latlng= json_encode(array('lat' => $lat, 'lng'=> $lng), JSON_FORCE_OBJECT);
                update_user_meta($customer->get_id(), 'shipping_szbd-picked-location', $latlng);
