		...
		init: function(loc,has_address) {
			...
			if (szbd.is_checkout == 1) {
				...
				jQuery(document)
					...
					.on('szbd_map_update_blocks', function (e, latlng,has_address) {
	...
	var szbd_map = {
		...
		initMap: function (
			...
			try {
				...
				if (from_marker ||  !szbd_checkout.has_address) {
					return;
				}
			...
			try {
				...
				if (from_marker == 'auto_marker') {
					...
					if (szbd_map.marker) {
						...
						szbd_map.marker = new google.maps.Marker(
							markerOptions
						);
			...
			try {
				if (
					...
					jQuery(document)
						...
						.on(
							'input.szbd2',
							'#billing-address_1,#billing-city,input#billing-state',
							szbd_map.remove_marker
						);
					jQuery(document)
						.off('change.szbd3')
						.on(
							'change.szbd3',
							'#billing-country,select#billing-state',
							szbd_map.remove_marker
						);
				}
			} catch (err) {
						...
						function (response) {
							...
							if (szbd.debug == 1) {
								...
								debug10 =	JSON.stringify(results) ;
									...
									console.debug(debug,debug2,debug3,debug4,debug5,debug6,debug7,debug8,debug9,debug10);
							}
