jQuery(function($) {
	...
	var szbd_checkout = {
		...
		init: function() {
			...
			if (szbd.is_checkout == 1) {
				if (szbd.precise_address === 'always') {
					...
					szbd_map.initMap(location, false, false, [], set_marker,null);
		...
		evaluate_server_response: function(response, from_marker, loc, geo_base) {
			...
			szbd_map.update_store_map(response.delivery_address, is_fail, is_precise_address, types, from_marker,response.faild);
		},
		try_geocode: function(from_marker, ok_types, geo_base, s_address, comp, has_address, isPlusCode) {
			...
			$.when(szbd_google_geocode.geocode(s_address, comp)).then(function(response) {
				...
				if (szbd.debug == 1) {
					console.debug(	'GEO CODE REQUEST:Address string:'  + JSON.stringify(location) + 'TYPES:' + 'none' + 'STATUS:' + status + 'GEOCODE:' + JSON.stringify(results) );
					}
	...
	var szbd_map = {
		...
		init_delivery_map: function() {
			...
			jQuery('#szbd_map').on('cma_placing_marker', function(event, location) {
				...
				szbd_map.remove_marker();
				szbd_map.update_store_map(location, false, true, null, false,null);
			});
		},
		
		initMap: function(uluru, is_fail, is_precise_address, types, from_marker,faild) {
			...
			try {
				...
				if ((szbd_map.updatingplusCode || !from_marker) && (typeof uluru == 'undefined' || uluru == null || uluru == false || _.isArray(uluru) || _.has(uluru, 'country') || _.isObject(faild) )) {
					
					if( _.isObject(faild)){
						let mapOptions =szbd_map.mapOptions;
						mapOptions.center = faild;
						mapOptions.zoom = szbd_map.get_zoom_level(faild.types);
						this.set_map(mapOptions, is_precise_address, faild, from_marker);

					}
					else if (_.isArray(uluru) || _.has(uluru, 'country')) {
		...
		update_store_map: function(loc, is_fail, is_precise_address, types, from_marker,faild) {
			szbd_map.initMap(loc, is_fail, is_precise_address, types, from_marker,faild);
		},
		...
		remove_marker_on_change: function() {
			if ($('#ship-to-different-address').find('input').is(':checked')) {
				jQuery(document).off('input.szbd2').on('input.szbd2', '#shipping_address_1,#shipping_city,input#shipping_state,input#shipping_postcode', szbd_map.remove_marker_only);
				jQuery(document).off('change.szbd3').on('change.szbd3', '#shipping_country,select#shipping_state', szbd_map.remove_marker_only);
			} else {
				jQuery(document).off('input.szbd2').on('input.szbd2', '#billing_address_1,#billing_city,input#billing_state,input#billing_postcode', szbd_map.remove_marker_only);
				jQuery(document).off('change.szbd3').on('change.szbd3', '#billing_country,select#billing_state', szbd_map.remove_marker_only);
			}
		},
        remove_marker_only:function(){
            jQuery('#szbd-picked').val('');
			jQuery('#szbd-plus-code').val('');
		...
		try_second_geocode: function(address, comp, mapOptions, uluru) {
			...
			if (szbd_checkout.only_country) {
				...
				$.when(szbd_google_geocode.geocode(address, comp)).then(function(response) {
					...
					if (szbd.debug == 1) {
						console.debug(	'2nd GEOCODE REQUEST:Address string:'  + address + 'Component restriction: ' + JSON.stringify(comp) + 'OK TYPES:' + 'none' + 'STATUS:' + status + 'GEOCODE:' + JSON.stringify(results) );
									}
